...
class End2End(nn.Module):
    ...
    def forward(self, images, pos_ref, neg_ref):
        resized_images = self.yolo(images, img_size=224)
        images_output = self.retrieval(resized_images)
        pos_ref_output = self.retrieval(pos_ref)
        neg_ref_output = self.retrieval(neg_ref)
        return images_output, pos_ref_output, neg_ref_output
